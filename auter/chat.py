    ...
    def run(self):
        ...
        while 1:
            ...
            if msg_recu=="fin":
                connec.close()
                connection.close()
                self.indicateur.setText(_translate("MainWindow","deconnecte", None))
                break
class Ui_MainWindow(object):
    ...
    def DemarerServeur(self):
        ...
        if server_lance==False:
            ...
            connection_avec_client,infos_connection=connection.accept()
            ...
            th.start()
    def recu(self,connec):
            msg_recu=connec.recv(1024).decode("Utf-8")
            print("Recu :",msg_recu)
            if msg_recu=="fin":
                connec.close()
                connection.close()
